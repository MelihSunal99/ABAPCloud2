CLASS zcl_05_duplicate_data_delete DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_05_DUPLICATE_DATA_DELETE IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    out->write( 'Nichts gelöscht - Schlüsselwerte sind nicht benachbart' ).
    ...
    DELETE ADJACENT DUPLICATES FROM flights.
    out->write( 'Contents after DELETE ADJACENT DUPLICATES' ).
    out->write( 'Nichts gelöscht - ABAP vergleicht alle Schlüsselwerte inklusive flight_date, die für jeden Eintrag unterschiedlich ist' ).
    out->write( flights ).
    out->write( ` ` ).
    ...
    out->write( 'Einträge mit identischen Werten carrier_id und connection_id wurden gelöscht' ).
    out->write( flights ).
  ENDMETHOD.
ENDCLASS.
